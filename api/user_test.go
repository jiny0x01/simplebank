...
import (
	...
	mockdb "github.com/jiny0x01/simplebank/db/mock"
	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				// 중요
				// createUser()가 호출되면서 생성된 hashedPassword랑
				// 테스트할 때 생성한 hashedPassword는 bcrypt 해쉬에 의해 일치하지 않음 util/password_test.go 참고
				// 따라서 util.checkPassword()로 확인해주는 작업이 필요
				// EqCreateUserParams에서 그 작업을 진행함
				// 구현은 gomock.Eq() 함수를 바탕으로 구현함.
